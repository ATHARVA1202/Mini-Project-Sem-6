...
model_filename = 'xgb_model_1.pkl'
...
def predict_manual():
    if request.method == 'POST':
        ...
        username_has_number = float(request.form['username_has_number'])
        full_name_has_number = float(request.form['full_name_has_number'])
        full_name_length = float(request.form['full_name_length'])
        is_private = float(request.form['is_private'])
        is_business_account = float(request.form['is_business_account'])
        has_external_url = float(request.form['has_external_url'])

        # Prepare the input data for prediction
        input_data = [[edge_followed_by, edge_follow, username_length, username_has_number,
                        full_name_has_number, full_name_length, is_private, is_business_account,
                        has_external_url]]
...
def predict_instagram_data():
    if request.method == 'POST':
        ...
        if scraped_data:
            # Prepare the input data for prediction
            input_data = [[
                # scraped_data["Username"],    # Number of followers
                scraped_data["edge_followed_by"],         # Number of followees
                scraped_data["edge_follow"],     # Length of username
                scraped_data["username_length"], # Whether username contains a number
                scraped_data["username_has_number"],# Whether full name contains a number
                scraped_data["full_name_has_number"],    # Length of full name
                scraped_data["full_name_length"],          # Whether the account is private
                scraped_data["is_private"],  # Whether the account was recently created
                scraped_data["is_business_account"],# Whether the account is a business account
                scraped_data["has_external_url"],          # Whether the account has guides
            ]]
            print(input_data)
            # Make predictions using the model
            prediction = xgb_classifier.predict(input_data)
